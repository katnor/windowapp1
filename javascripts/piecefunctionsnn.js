...
function drawmonthsbymn() {
	...
	var loader1 = new PxLoader(); 
 		for (i = 0; i < 7; i ++ ) {
			url = 'http://www.novamara.com/window/vlrallsounds/' + soundNames[month][i] + '.mp3';
			loader1.addSound(soundIds[(month*7)+i], url);  
			console.log("new sounds is...." + soundIds[(month*7) + i]);
			}
	     loader1.start()
		 loader1.addProgressListener(function(e) { 
			if (e.completedCount == e.totalCount) {
			for (i = 0; i < 7; i ++ ) {
				spacer = Math.floor((w*0.9)/7)*i;
				tobjs[i] = drawImage(soundIds[ (month*7) + i], w, h, spacer, stage);
				console.log("objs is..." + tobjs[i]);
				}	
			}
		}); 
 	return (tobjs);
  ...
  k = 0;
	...
	if(initmonth == 0) {
		for(i = 0; i < 7; i++) {
			...
			ilay.add(tobjs[i][0]);
			ilay.draw();
			loopSound(soundIds[(month*7)+i], tobjs[i][0], 1);
			oldobjects[i] = tobjs[i];
		}
	  initmonth = 1;
	  ...
	  oldtime = nowtime;
		...
		for(k = 0; k < 7; k++) {
			...
			console.log("Old obj sound..." + oldobjects[k][2]);
		}
