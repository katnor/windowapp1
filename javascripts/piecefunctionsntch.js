...
//when sounds have loaded move to buttons and instructions
 function son() {		
 	 		//	stoppit = 1;
$("#loaderwhitestrip").fadeTo(800,0); //white bar
$("#loadergreystrip").fadeTo(700,0); //grey bar
setTimeout(function() {$("#loadergreystrip").remove();$("#loaderwhitestrip").remove();}, 50);		
...
//MOUSE EVENTS
	...
	moveImg.on("touchstart",  function(e){
	...
	moveImg.on("touchend", function(e){
	...
	moveImg.on("touchmove", function(e){
		drag = true;
		e.stopPropagation();
		xval = moveImg.getX();
		yval = moveImg.getY(); 
...
function drawWords(txt1, w1, h1, x1, y1, stage, color) {
  ...
  shaper.on("touchstart", function(e){
	e.stopPropagation();
...
function ch(month, tobjs, ilay) {
  ...
  k = 0;
	if(initmonth == 0) {	
		...
		ilay.add(tobjs[i][0]);
		ilay.draw();
		loopSound(source[i], tobjs[i][0], 1);
